...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-foot {
    ...
    border-top: 1px solid #EDEEF8; flex-wrap: wrap; align-items: center;
  }
  .btn {
...
<div class="card">
  ...
  {{-- Action buttons --}}
  <div class="card-foot">
    <a href="{{ route('teacher.results.index') }}?term_id={{ $term->id }}" class="btn btn-back">
      &#8592; Go Back
    </a>

    {{-- Repost the generate form with confirm=1 --}}
    <form action="{{ route('teacher.results.generate') }}" method="POST" style="flex:1;display:flex;">
      @csrf
      <input type="hidden" name="student_id" value="{{ $student->id }}">
      <input type="hidden" name="term_id"    value="{{ $term->id }}">
      <input type="hidden" name="confirm"    value="1">
      <button type="submit" class="btn btn-proceed" style="width:100%;">
        &#8681; Download Anyway (Incomplete)
      </button>
    </form>

    <span class="proceed-note">
      The PDF will include only the approved/locked scores. Missing subjects will not appear.
    </span>
  </div>
